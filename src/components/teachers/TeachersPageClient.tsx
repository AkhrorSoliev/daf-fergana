...
function getImageSrc(slug: string, branch: string): string {
  if (branch === "qarshi") {
    // Qarshi teachers have images in /lehrer-qarshi/
    const ext =
      slug === "herr-khasan" ? "jpg" :
      slug === "herr-lazizjon" ? "jpg" :
      "JPG";
    return `/lehrer-qarshi/${slug}.${ext}`;
  }
  // Fergana teachers
  if (slug === "herr-umarov") return "/lehrer/herr-musinjon.png";
  if (slug === "herr-doston") return "/lehrer/herr-doston.jpg";
  if (slug === "frau-sakina") return "/lehrer/frau-sakina.jpg";
  return `/lehrer/${slug}.png`;
}

export default function TeachersPageClient() {
  const { t, locale } = useI18n();
  const [selectedBranch, setSelectedBranch] = useBranchParam();

  const filteredTeachers = useMemo(() => {
    const filtered = teachers.filter((t) => t.branch === selectedBranch);

    if (selectedBranch === "fergana") {
      // Reorder: move herr-doston and frau-sakina after herr-umarov
      const moveSlugs = new Set(["herr-doston", "frau-sakina"]);
      const umarovIndex = filtered.findIndex((t) => t.slug === "herr-umarov");
      if (umarovIndex === -1) return filtered;
      const toMove = filtered.filter((t) => moveSlugs.has(t.slug));
      const without = filtered.filter((t) => !moveSlugs.has(t.slug));
  ...
  // Determine featured teacher per branch
  const featuredSlug =
    selectedBranch === "fergana" ? "herr-jamsher" : "herr-lazizjon";
      ...
      <div className="container py-12 md:py-20 relative">
        ...
        >
          <Select value={selectedBranch} onValueChange={setSelectedBranch}>
            <SelectTrigger className="w-64 h-12 text-base border-border/60 focus:border-accent bg-white/90 dark:bg-card">
              <SelectValue placeholder={t("teachers.selectBranch")} />
            </SelectTrigger>
            <SelectContent>
              {branches.map((branch) => (
                <SelectItem
        ...
        >
          {filteredTeachers.map((teacher, index) => {
            const isFeatured = teacher.slug === featuredSlug;
            const imageSrc = getImageSrc(teacher.slug, teacher.branch);
            return (
              <motion.div
                key={teacher.slug}
